...
public class SupporterScene extends PixelScene {
	...
	public void create() {
		...
		IconTitle title = new IconTitle(Icons.DISPLAY.get(), Messages.get(this, "title"));
		title.setSize(200, 0);
		title.setPos(
				(w - title.reqWidth()) / 2f,
				(20 - title.height()) / 2f
		...
		StyledButton link = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(this, "supporter_link")){
			...
			protected void onClick() {
				super.onClick();
				String link = "https://www.github.com/H-Chris233";
		...
		link.icon(Icons.get(Icons.DISPLAY));
	...
	private static class SupporterMessage extends Component {
		...
		protected void createChildren() {
			...
			if (Messages.lang() != Languages.CHINESE) {
				message += "\n" + Messages.get(SupporterScene.class, "patreon_chinese");
			}
			message += "\n\n- H-Chris233";
			...
			icon = Icons.get(Icons.BACKPACK_LRG);
			add(icon);
